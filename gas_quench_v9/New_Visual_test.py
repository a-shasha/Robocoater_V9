print('New Visual Test for Self-Driving (V9_GQ alt report)')
...
OUTPUT_SUBFOLDER_DEFAULT = "New_Visual_GQ_output"
REFLECTION_TRACE_WAVELENGTHS_NM = [700.0, 600.0, 750.0, 800.0]

# BO response-surface map settings
BO_X_RANGE = (7.0, 70.0)    # gas quench start time
BO_Y_RANGE = (1.0, 14.0)    # gas quench duration
BO_GRID_N = 420
...
def _calculate_utility_metrics(master_dict):
    """Standalone V9_GQ metric extraction for plotting/reporting."""
    tags = master_dict.get('Tags', {})
    params = master_dict.get('Parameters', {})
    film_score = master_dict.get('Film Ranking', {}).get('Score', 0.0)

    gq_time = float(tags.get('Gas Quench Start Time', params.get('Gas Quench Time', 0)))
    gq_duration = float(tags.get('Gas Quench Duration', params.get('Gas Quench Duration', 0)))

    norm_gq_time = ((gq_time - BO_X_RANGE[0]) / (BO_X_RANGE[1] - BO_X_RANGE[0])) * 2 - 1
    norm_gq_duration = ((gq_duration - BO_Y_RANGE[0]) / (BO_Y_RANGE[1] - BO_Y_RANGE[0])) * 2 - 1

    results = {
        'parameter_list': [gq_time, gq_duration],
        'norm_parameters_list': [norm_gq_time, norm_gq_duration],
        'gqTime': gq_time,
...
def _extract_campaign_point(master_dict, file_name):
    ...
    gq_time = _as_float(
        tags.get('Gas Quench Start Time', params.get('Gas Quench Time')),
        default=None,
    )
    gq_duration = _as_float(
        tags.get('Gas Quench Duration', params.get('Gas Quench Duration')),
        default=None,
    )
    ...
    if gq_time is None or gq_duration is None or rank_num is None or objective_value is None:
        return None

    # Downstream BO plotting helpers still use legacy x/y key names.
    # In the GQ workflow those keys represent gas-quench time and duration.
    return {
        'name': file_name,
        'drip_time': float(gq_time),
        'drip_duration': float(gq_duration),
        'rank': int(rank_num),
...
def plot_campaign_surface(
    ...
    ax.set_xlim(*x_range)
    ax.set_ylim(*y_range)
    ax.set_xlabel('GQ Time (s)', fontsize=6.4)
    ax.set_ylabel('GQ Duration (s)', fontsize=6.4)
...
def _generate_plots_new(
    ...
    try:
        ...
        tags = master_dict.get('Tags', {})
        gq_time = results.get('gqTime', tags.get('Gas Quench Start Time'))
        ...
        title_fig = (
            f"{file_name} | GQ Time: {_fmt(gq_time)} s | "
        ...
        drip_idx_pl = 0
        drip_idx_abs = 0
        if pl_times and gq_time is not None:
            drip_idx_pl = min(range(len(pl_times)), key=lambda i: abs(pl_times[i] - gq_time))
        if abs_times and gq_time is not None:
            drip_idx_abs = min(range(len(abs_times)), key=lambda i: abs(abs_times[i] - gq_time))

        snapshot_targets = _drip_anchored_targets(gq_time, gq_duration)
        ...
        if pl_selected and dd_pl_correct and wavelengths_pl:
            for k in pl_selected:
                if k in dd_pl_correct:
                    plt1.plot(wavelengths_pl, dd_pl_correct[k])
            leg1 = plt1.legend([f"{k:.2f}" for k in pl_selected], prop={'size': 5}, title='Times')
            if leg1 is not None:
        ...
        if pl_times_full and energies_pl and dd_pl_contour:
            ...
            if gq_time is not None:
                plt2.axvline(float(gq_time), color='k', linestyle='--', linewidth=1.2)
        ...
        if peak_energy and pl_times:
            ...
            if y_e.size > 0:
                ...
                plt3a.set_xlabel('Time (seconds)', fontsize=8)
            else:
        ...
        if peak_area and pl_times:
            ...
            if y_a.size > 0:
                ...
                plt3b.set_xlabel('Time (seconds)', fontsize=8)
        ...
        if refl_selected and dd_reflection_w_plot and wavelengths_abs:
            ...
            leg4 = plt4.legend([f"{k:.2f}" for k in refl_selected], prop={'size': 5}, title='Times')
            if leg4 is not None:
        ...
        # Plot 5: 400-900 and gas-quench start line
        if dd_reflection_contour and wavelengths_abs:
            ...
            if np.isfinite(Z).any():
                ...
                if gq_time is not None:
                    plt5.axvline(float(gq_time), color='k', linestyle='--', linewidth=1.2)
        ...
        for i, target_nm in enumerate(REFLECTION_TRACE_WAVELENGTHS_NM):
            ...
            if x_r.size > 0 and y_r.size > 0:
                x_plot = x_r[drip_idx_abs:] if drip_idx_abs < x_r.size else x_r
                y_plot = y_r[drip_idx_abs:] if drip_idx_abs < y_r.size else y_r
...
def _load_points_from_campaign_experiments(campaign_folder):
    ...
    for name in sorted(campaign_dict.keys(), key=_sample_sort_key):
        ...
        if not isinstance(actions, (list, tuple)) or len(actions) < 2:
            continue
        gq_time = _as_float(actions[0], default=None)
        gq_duration = _as_float(actions[1], default=None)
        obs = _as_float(item.get('Observation'), default=None)
        if gq_time is None or gq_duration is None or obs is None:
            continue
        if not np.isfinite(gq_duration):
            continue
